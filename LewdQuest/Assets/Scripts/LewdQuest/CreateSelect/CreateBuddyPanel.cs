...
public class CreateBuddyPanel : MonoBehaviour {
	...
	public void create(Attack attack , Attack.TYPE  type){

		Debug.Log (" creating "  + attack.type + attack.getName());
		switch (attack.type) {

		case Attack.TYPE.MAGIC:
			createDamagePanel (attack);
			break;
		case Attack.TYPE.DAMAGE:
			createDamagePanel (attack);
			break;
		case Attack.TYPE.HEAL:
			createHealPanel (attack);
			Debug.Log (" creating heal ");
			break;
	...
	void createDamagePanel(Attack attack){
		ArrayList enemyMap = manager.getEnemyMap ();
		...
		if (enemyMap.Count > 1 & aliveCount () > 1) {
			...
			for (int x = 0; x < enemyMap.Count; x++) {
				...
				if (character.getAlive ()) {
					...
					buttonPopUp.GetComponent<Button> ().onClick.AddListener (() => {
						...
						selectAction (attack , int.Parse (buttonPopUp.name),false);
			...
			selectAction(attack , getLastAlivePosition(),false);
		}
	...
	void createHealPanel(Attack attack){
			...
			for (int x = 0; x < partyMap.Count; x++) {
				...
				if (character.getAlive ()) {
					...
					buttonPopUp.GetComponent<Button> ().onClick.AddListener (() => {
						...
						enableOldGui ();
						onHoverExit (int.Parse (buttonPopUp.name));
						selectAction (attack , int.Parse (buttonPopUp.name),true);
	...
	void selectAction(Attack attack , int pos, bool isPlayer){
		...
		case Attack.TYPE.MAGIC:
			manager.dealDamageBuddy (attack, pos);
			break;
		case Attack.TYPE.DAMAGE:
			manager.dealDamageBuddy (attack, pos);
			break;
		case Attack.TYPE.HEAL:
			manager.useHealBuddy (attack, pos, isPlayer);
			break;
			/*
		case Attack.TYPE.LEWD:
...
*/
		...
		manager.activateRegularButtons ();
