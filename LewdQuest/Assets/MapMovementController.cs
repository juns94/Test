...
public class MapMovementController : MonoBehaviour {
	...
	void Update () {
		...
		if (Input.GetKey(KeyCode.W)) {
			got.localPosition= new Vector3 (got.localPosition.x, got.localPosition.y + move); moving = false;
		}
		...
		if (Input.GetKey(KeyCode.A)) {
			...
			got.localPosition = new Vector3 (got.localPosition.x - move, got.localPosition	.y );moving = false;
		}
	...
	//	coll.gameObject.transform.parent.GetChild(0).gameObject.SetActive (true);
	...
	//	coll.gameObject.transform.parent.GetChild (0).gameObject.SetActive (false);
